...
class BacktestingEngine(object):
    ...
    def __init__(self):
        ...
        self.cachePath = os.path.join(os.path.expanduser("~"), ".backtest_data")       # 本地数据缓存地址
    ...
    #----------------------------------------------------------------------
    def roundToPriceTick(self, price,vtSymbol):
        """取整价格到合约最小价格变动"""
    ...
    def setPriceTick(self, priceTick = 0):
        ...
        if len(priceTick) > len(self.symbolList):
            ...
            for i, sym in enumerate(self.symbolList):
                self.priceTickDict[sym] = priceTick[sym]
    ...
    def loadHistoryData(self, symbolList, startDate, endDate=None,fp=False):
        ...
        for sym in symbolList:
            ...
            dataList += [self.parseData(dataClass, item.to_dict()) for _, item in
                         data.iterrows()]
            if fp==True:
                data.to_csv('E:\\'+start[:8]+end[:8]+'%s.csv'%(sym))
    ...
    def runBacktesting(self,fp=False):
        ...
        self.backtestData = self.loadHistoryData(self.strategy.symbolList, start, end,fp)
            ...
            if self.strategy.mainContract_changeDate[sym]:
                if data.datetime==self.strategy.mainContract_changeDate[sym][0]:
                    ...
                    if self.strategy.posDict[sym + '_LONG'] != 0:
                        print('多换合约',self.strategy.posDict[sym+'_LONG'],self.dt)
                        self.sendOrder(sym, CTAORDER_SELL, temp.close, self.strategy.posDict[sym+'_LONG'],
                                        PRICETYPE_MARKETPRICE, self.strategy)
                        print('多换合约平',temp.datetime, temp.open,temp.high,temp.low,temp.close,self.strategy.posDict[sym+'_LONG'])
                        self.dt=temp.datetime
                        self.crossLimitOrder(temp)  # 先撮合限价单
                        self.strategy.cancelAll()
                        self.strategy.change_Long[sym]=True
                    if self.strategy.posDict[sym+'_SHORT']!=0:
                        print('空换合约',self.strategy.posDict[sym + '_SHORT'],self.dt)
                        self.sendOrder(sym, CTAORDER_COVER, temp.close, self.strategy.posDict[sym + '_SHORT'],
                                        PRICETYPE_MARKETPRICE, self.strategy)
                        self.dt=temp.datetime
                        print('空换合约平',temp.datetime,temp.open,temp.high,temp.low,temp.close,self.strategy.posDict[sym + '_SHORT'])
                        
                        self.crossLimitOrder(temp)  # 先撮合
                        self.strategy.cancelAll()
                        self.strategy.change_Short[sym] = True
                    del self.strategy.mainContract_changeDate[sym][0]
                    self.output(u'策略%s更换合约,当前主力合约%s' % (data.date,data.vtSymbol))
    ...
    def sendOrder(self, vtSymbol, orderType, price, volume, priceType, strategy):
        ...
        if priceType == PRICETYPE_LIMITPRICE:
            order.price = self.roundToPriceTick(price,vtSymbol)
        elif priceType == PRICETYPE_MARKETPRICE and order.direction == DIRECTION_LONG:
            order.price = self.roundToPriceTick(price,vtSymbol) * 1000
    ...
    #----------------------------------------------------------------------
    def loadBar(self, symbol,Datetime,barnum=None):
        """直接返回初始化数据列表中的Bar"""
        col = self.db[symbol]
    ...
    def calculateBacktestingResult(self):
            ...
            if trade.direction == DIRECTION_LONG:
                ...
                # 当前多头交易为平空
                    ...
                    while True:
                        ...
                        result = TradingResult(trade.vtSymbol, entryTrade.price, entryTrade.dt, 
                                               exitTrade.price, exitTrade.dt,
                                               -closedVolume, self.rate, self.slippage, self.sizeDict[trade.vtSymbol])
        ...
        for symbol, tradeList in longTrade.items():
            ...
            for trade in tradeList:
                result = TradingResult(trade.vtSymbol,trade.price, trade.dt, endPrice, self.dt,
                                       trade.volume, self.rate, self.slippage, self.sizeDict[symbol])
        ...
        for result in resultList:
            ...
            holdingList.append(holdingdays.days)
        # 计算盈亏相关数据
    ...
    def showBacktestingResult(self):
        ...
        self.output(u'多仓盈亏：\t%s' %formatNumber(sum(d['longPnl'])))
        self.output(u'空仓盈亏：\t%s' %formatNumber(sum(d['shortPnl'])))
        ...
        if self.logActive:
            ...
            if not os.path.isdir(save_path):
                os.makedirs(save_path)
            filename = os.path.join(save_path, u"BacktestingResult.csv" )
            ddresult.to_csv(filename)
            self.output(u'BacktestingResult已保存') 
        ...
        if self.logActive:
            ...
            if not os.path.isdir(save_path):
                os.makedirs(save_path)
            filename = os.path.join(save_path, u"回测统计图-2.png" )
            plt.savefig(filename)
            self.output(u'策略回测统计图-2已保存') 
        plt.show()
    ...
    def runParallelOptimization(self, strategyClass, optimizationSetting):
        ...
        for setting in settingList:
            ...
            l.append(pool.apply_async(optimize, (strategyClass, setting,
                                                 ...
                                                 self.slippage, self.rate, self.sizeDict, self.priceTick,
    ...
    def calculateDailyResult(self):
        ...
        for symbol, resultDictByDay in self.dailyResultDict.items():
            ...
            for dailyResult in resultDictByDay.values():
                ...
                dailyResult.calculatePnl(openPosition, self.sizeDict[symbol], self.rate, self.slippage)
